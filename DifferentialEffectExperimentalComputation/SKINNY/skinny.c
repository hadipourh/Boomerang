...
int difference(int r, int ver, int N3, unsigned char *dp, unsigned char *dc, unsigned char *dk1)
{
    int i;
...
double send_difference(int R, int ver, int N1, int N2, int N3, unsigned char *dp, unsigned char *dc, unsigned char *dk1)
{
    // Parallel execution
    int NUM[N1];
    int counter;
    ...
    for (counter = 0; counter < N1; counter++)
    {
        ...
        for (int j = 0; j < N2; j++)
        {
            num += difference(R, ver, N3, dp, dc, dk1);
        }
        NUM[ID] = num;
    }
...
int main()
{
    ...
    int n = 5;   // Number of independent experiments
    ...
    char dp_str[] = "00000000000000000000000000000000";
    char dc_str[] = "40400040004000000000184000400040";   
 
    char dk1_str[] = "00000000000000000000f8000000000000000000000000000000fc0000000000";
    // #######################################################################################################
    // #######################################################################################################
    ...
    int N3 = 1 << 8;      // Number of queries per bunches : N3
    //################### Number of total queries : N1*N2*N3 ###############
    double sum = 0;
    for (int i = 0; i < n; i++)
    {
        sum += send_difference(R, ver, N1, N2, N3, dp, dc, dk1);
    }
