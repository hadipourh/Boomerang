...
void init_prng(int offset) {
    ...
    unsigned int initial_seed = 1000*time(NULL) + offset;
...
double verify(int experiment_number, uint8_t dp[16], uint8_t dc[16], uint8_t R, int N1, UINT64 N2, UINT64 N3) {
	...
	for(counter = 0; counter < N1; counter++){
        ...
        init_prng(ID + experiment_number);
		UINT64 num = 0;
        uint8_t tk[4][16];
...
double verify_random_subtks(int experiment_number, uint8_t dp[16], uint8_t dc[16], uint8_t R, int N1, UINT64 N2, UINT64 N3) {
    int *NUM;
...
int main() {
    ...
    const int n = 1000;//number of repetition
    ...
    for(int i = 0; i < n; i++) {
        if (random_subtks == true){
            sum += verify_random_subtks(i, dp, dc, R, N1, N2, N3);
        }
        else{
            sum += verify(i, dp, dc, R, N1, N2, N3);
        }
    }
