...
**We have observed that a paper published in ToSC-2022-4 [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) has examined certain characteristics found in our GitHub repository. In the following sections, we clarify that the local/global dependencies discussed in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) have no impact on our methods or results.**

- **First, the characteristics mentioned in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) are intermediate outputs generated in the initial stage of our method, which are subsequently discarded. As specified in our paper [[2]](https://ia.cr/2020/1317), our tool disregards these characteristics in later stages (i.e., when calculating the differential effect and executing the boomerang switch). Specifically, when our tool derives an `R+6`-round upper (resp. lower) trail at the initial step, it omits the last (resp. first) 6 rounds of this trail and only retains the first (resp. last) `R` rounds. Unfortunately, this fact was not acknowledged in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976).**

- **Additionally, in the initial (resp. final) `R` rounds, we rely exclusively on differentials rather than characteristics. This clarification emphasizes two key points: first, we exclude the last (resp. first) 6 rounds of the derived upper (resp. lower) trails; second, even within these initial (resp. final) `R` rounds, we only utilize differentials, not characteristics. However, all assertions in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) involve the full characteristics generated during the initial phase of our tool (e.g., 12-, 13-, 14-round characteristics). For instance, figure 4 in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) depicts the first 6 rounds of a lower trail from our GitHub repository—a portion that our tool bypasses entirely and that we do not use. Thus, we clarify that none of the characteristics cited in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) play a substantive role in our method. Moreover, all characteristics attributed to our work in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) were derived from residual intermediate outputs in our GitHub repository. These residual characteristics vary randomly with each execution of our tool and are not integral to our analysis. As indicated in the table of distinguishers in our paper, we fix differences in only four specific positions.**

 - **Since the approach in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) does not apply to boomerang distinguishers, we offer some insights into the validity of our boomerang distinguishers. All local/global dependencies discussed in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) occur solely within the last (resp. first) 6 rounds of the upper (resp. lower) trails, which are excluded from our method. These segments are part of the boomerang switch and should be treated as part of a smaller boomerang distinguisher. We have experimentally validated the middle segment (boomerang switch) in our sandwich distinguishers.**

 - **Furthermore, the differentials through `E0` and `E1` in our boomerang distinguishers hold `zero` values almost entirely. Thus, the local/global dependencies mentioned in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) do not appear in differentials through `E0` and `E1`. We also analyzed the dependency between upper and lower differentials across the 6-round middle segment. Therefore, our boomerang distinguishers remain valid, potentially with a higher probability than claimed in our paper. As clear evidence, we have provided numerous right quartets (in this repository and in our paper) for several of our proposed boomerang distinguishers.**

**Initially, we removed these residual characteristics from our repository to avoid potential confusion, as they are never used in our paper or method. However, we later chose to restore them for the sake of transparency, given their reference in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976). It is worth noting that the authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) did not consult us prior to publishing their claims, which is regrettable. After noticing the claims in the published version of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976), we reached out to the authors to clarify these facts. They eventually added a sentence in the ePrint version of their paper stating that their claims do not invalidate our boomerang distinguisher; however, the ToSC version of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) remained unchanged.**

**In our [FSE 2023 talk](https://iacr.org/submit/files/slides/2023/fse/fse2023/tosc2022_3_36/slides.pdf), we proved that the dependency issues discussed in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) have no impact on the validity of the boomerang distinguishers. To this end, we employed two differential trails to construct an 11-round deterministic boomerang distinguisher for SKINNY-TK2. According to [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976), the differential trails in our example should be impossible. However, the resulting boomerang distinguisher succeeds for all keys/plaintexts with a probability of one.**
