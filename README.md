...
- The authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) have claimed that some characteristics in our work are impossible. All of their claims are about the characteristics of more than 10 rounds lengths. It clarifies their apparent misunderstanding since we never use a differential of more than 9 rounds as a building block for our distinguishers.
- The 6-round middle part of our distinguishers is treated as a boomerang switch where the dependency between two trails makes sense, not the dependency in a single trail. For example, based on their wrong interpretation, they have claimed that no key satisfies the lower differential in our boomerang distinguisher II for SKINNY-64-128. However, we have provided many right quartets for this distinguisher and verified that it works for all keys with a probability of $2^{-38}$. This should clarify their apparent misunderstanding. It also shows that the dependency in one of the upper or lower differentials in the boomerang switch (middle part of sandwich distinguishers) has almost no impact on the resulting boomerang distinguishers. What matters is actually the dependency between two differentials in the middle that have been completely considered in our work.
...
- The authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) admitted that their claims does not invalidate our work (in a very short sentence setion 4.3, the last line of the first paragprah). However, this disclaimer have not been clarified in their table of results. In addition, this desclaimer only carifies that we use differential (not differential characteristics). This disclaimer does not clarify that their interpretation from the differentials is also wrong. Because, we never treat the middle part of sandwich distinguisher as a siple differential, and we treat it as a boomerang switch where the only the dependency in two sides of boomerang makes sence not the dependency in a single differential.
- Another important fact that is not mentioned in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976), is that we never fix the difference values in the middle part of the sandwich distinguisher.
- Another misrepresentation is that the authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) put our work besides some other works that are focused on finding simple differential characteristics. The authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) never point to the fact of how we use these characteristics in our methods. Our tool derives some characteristics as an intermediate output. However, we never use these intermediate outputs in our method. According to our paper, we truncate these characteristics and include the middle 6 rounds into the boomerang switch, where we take all dependencies between two sides of the boomerang into account. Thus, we only use differentials of 8 to 9 rounds in length in a single side of our boomerang distinguishers. 
