...
- The authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) have claimed that some characteristics in our work are impossible. All of their claims are about the characteristics of more than 10 rounds lengths. However, we never use a differential of more than 9 rounds as a building block for our distinguishers. The 6-round middle part of our distinguishers is treated as a boomerang switch where the differences in **two different sides** of boomerang are fixed. Therefore, the middle 6-round in our distinguishers can not be treated as a part of a simple differential. This is the main misunderstaing of our work by the authors of [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976). In other words, for the middle part, only the the input difference of the upper and lower trails are fixed and the output differences are totally free. In this case, talking about a simple differential (characteristics) for the middle part does not make sense. It should clarify what is mainly wrong with the claims in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976). 
- [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976) is only focused on dependency in a simple differential trail, where for the middle part of our sandwich distinguisher, the dependency between two trails makes sense, not the dependency in a single trail. For example, based on their wrong interpretation, they have claimed that no key satisfies the lower differential in our boomerang distinguisher II for SKINNY-64-128. However, we have provided many right quartets for this distinguisher (in our paper and this GitHub repository) and verified that it works for all keys with a probability of $2^{-38}$. This should clarify their apparent misunderstanding. It also shows that the dependency in one of the upper or lower differentials in the boomerang switch (middle part of sandwich distinguishers) has almost no impact on the resulting boomerang distinguishers. What matters is actually the dependency between two differentials in the middle that have been completely considered in our work. These facts have not been mentioned in [[1]](https://tosc.iacr.org/index.php/ToSC/article/view/9976), which is unfortunate.
