...
class Skinnyrk:
    ...
    def constraints_of_trunc_xor(self, a, b, c):
        """
        a + b + c >= 2 d
        d >= a
        d >= b
    ...
    def constraints_of_mixing_layer(self, x, y):
        ...
        for j in range(4):
            ...
            self.constraints_of_trunc_xor(x[j + 4*0], x[j + 4*2], dummy_var)
            self.used_variables.append(dummy_var)
            self.dummy_var_counter += 1
            self.constraints_of_trunc_xor(dummy_var, x[j + 4*3], y[j + 4*0])
            self.constraint_of_equality(x[j + 4*0], y[j + 4*1])
            self.constraints_of_trunc_xor(x[j + 4*1], x[j + 4*2], y[j + 4*2])
    ...
    def gen_objective_function(self):
        ...
        A = ["xu_%d_%d" % (r, i) for r in range(
            0, (self.R1 - self.R)) for i in range(16)]
        B = ["xl_%d_%d" % (r, i) for r in range(self.R, self.R2)
             for i in range(16)]
...
########################################################################################################
    ...
    def constraints_of_upper_trail(self):
        ...
        zero_link = ["s_%d_%d" % (r, i)
                     for r in range(self.R) for i in range(16)]
        ...
        for r in range(0, rounds):
            ...
            for i in range(16):
                ...
                if (i < 8):
                    ...
                    for j in range(8):
                        self.constraints_of_exact_xor(
                            tk1[i][j], tk2[i][j], round_tk[i][j])
                else:
                    for j in range(8):
                        self.constraint_of_equality(tk2p[i][j], next_tk2[i][j])
            # Linking bit-wise variables to word-wise ones
            for i in range(8):
                self.constraints_of_linking_word_to_bits(
                    round_tkw[i], round_tk[i])
            # Constraints by ART
            for j in range(4):
                self.constraints_of_trunc_xor(
                    x_in[j + 4*0], round_tkw[j + 4*0], y[j + 4*0])
                self.constraints_of_trunc_xor(
                    x_in[j + 4*1], round_tkw[j + 4*1], y[j + 4*1])
...
########################################################################################################
    ...
    def get_bitwise_state_values_at_round(self, s, r):
        ...
        v_values = [hex(int("".join(v_values[i]), 2))[2:].zfill(2)
                    for i in range(16)]
