...
class Skinnyrk:
    ...
    def constraints_of_tk2_lfsr(self, x, y):
        """
    ...
    def constraints_of_tk3_lfsr(self, x, y):
        """
        ...
        self.constraints_of_exact_xor(x[0], x[3], y[0])    
...
########################################################################################################
    ...
    def constraints_of_upper_trail(self):
        ...
        for r in range(0, rounds):
            ...
            tk3p = self.permute_tweakey_nibbles(tk3)
            for i in range(16):
                for j in range(4):
                    self.constraint_of_equality(tk1p[i][j], next_tk1[i][j])
                if (i < 8):
                    self.constraints_of_tk2_lfsr(tk2p[i], next_tk2[i])
                    self.constraints_of_tk3_lfsr(tk3p[i], next_tk3[i])
                    for j in range(4):                        
...
########################################################################################################
    ...
    def retrieve_upper_trail(self, fileobj_diffpath_up):
        ...
        cipher_parameters += 'tweaksize: 128\n'
        ...
        temp = list(tku3_values)
        for j in range(16):
            if (temp[j] == '0'):
                fileobj_diffpath_up.write("- tk20[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))        
    ...
    def retrieve_lower_trail(self, fileobj_diffpath_down):
        ...
        temp = list(tkl3_values)
        for j in range(16):
            if (temp[j] == '0'):
                fileobj_diffpath_down.write("- tk20[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))        
