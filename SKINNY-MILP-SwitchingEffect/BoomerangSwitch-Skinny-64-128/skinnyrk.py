...
tk : bit-wise
tkw : word-wise
tk and tkw are linked towgether via the following condition
u vs d
tkw_i_0 >= tk_i_0_0, tkw_i_0 >= tk_i_0_1, tkw_i_0 >= tk_i_0_2, tkw_i_0 >= tk_i_0_3,
tkw_i_0 <= tk_i_0_0 + tk_i_0_1 + tk_i_0_2 + tk_i_0_3,
...
class Skinnyrk:
    def __init__(self, r0, r1, rm, w0, w1, wm, number_of_patterns, time_limit):
        self.permute_nibble_table = [
            0x0, 0x1, 0x2, 0x3, 0x7, 0x4, 0x5, 0x6, 0xa, 0xb, 0x8, 0x9, 0xd, 0xe, 0xf, 0xc]
        self.permute_tweak_nibble_table = [
    ...
    def gen_bit_wise_variables(self, r, s):
        ...
        """
        variables = [["%s_%d_%d_%d" % (s, r, i, j) for j in range(4)] for i in range(16)]
        for i in range(16):
    ...
    def gen_half_bit_wise_variables(self, r, s):
        ...
        """
        variables = [["%s_%d_%d_%d" % (s, r, i, j) for j in range(4)] for i in range(8)]
        for i in range(8):
            self.used_variables.extend(variables[i])
        return variables
    ...
    def constraints_of_truncxor(self, a, b, c):        
        """
        a + b + c >= 2 d
        d >= a
        d >= b
    ...
    def constraints_of_mixing_layer(self, x, y):
        ...
        for j in range(4):
            ...
            self.constraints_of_truncxor(x[j + 4*0], x[j + 4*2], dummy_var)
            self.used_variables.append(dummy_var)
            self.dummy_var_counter += 1
            self.constraints_of_truncxor(dummy_var, x[j + 4*3], y[j + 4*0])
            self.constraint_of_equality(x[j + 4*0], y[j + 4*1])
            self.constraints_of_truncxor(x[j + 4*1], x[j + 4*2], y[j + 4*2])
            self.constraint_of_equality(dummy_var, y[j + 4*3])
        
    def constraints_of_lfsr(self, x, y):
        """
        Generate constraints related to LFSR
        It is supposed that x, and y are arrays of lentgh 4, where each entry depicts one bit of difference
        """
        for i in range(3):
            self.constraint_of_equality(y[i], x[i + 1])
        self.constraints_of_exact_xor(x[0], x[1], y[3])
    ...
    def permute_nibbles(self, state):
        temp = [0]*16
        for i in range(16):
            temp[i] = state[self.permute_nibble_table[i]]
        return temp
    
    def permute_tweakey_nibbles(self, state):
        temp = [0]*16
        for i in range(16):
            temp[i] = state[self.permute_tweak_nibble_table[i]]
        return temp 
    ...
    def gen_objective_function(self):
        ...
        A = ["%d xu_%d_%d" % (self.w0, r, i) for r in range(0, (self.R1 - self.R)) for i in range(16)]
        B = ["%d xl_%d_%d" % (self.w1, r, i) for r in range(self.R, self.R2) for i in range(16)]
        C = ["%d s_%d_%d" % (self.wm, r, i) for r in range(self.R) for i in range(16)]
        if (A != []) and (B != []):
...
########################################################################################################
    ...
    def constraints_of_upper_trail(self):
        ...
        temp = " + ".join(x_in + tk1_flat + tk2_flat) + " >= 1\n"
        fileobj.write(temp)
        fileobj.close()
        for r in range(0, rounds):
            ...
            tk1p = self.permute_tweakey_nibbles(tk1)
            tk2p = self.permute_tweakey_nibbles(tk2)
            for i in range(16):
                for j in range(4):
                    self.constraint_of_equality(tk1p[i][j], next_tk1[i][j])
                if (i < 8):
                    self.constraints_of_lfsr(tk2p[i], next_tk2[i])
                    for j in range(4):
                        self.constraints_of_exact_xor(tk1[i][j], tk2[i][j], round_tk[i][j])
                else:
                    for j in range(4):
            ...
            ## Constraints by ART
            for j in range(4):
                self.constraints_of_truncxor(x_in[j + 4*0], round_tkw[j + 4*0], y[j + 4*0])
                self.constraints_of_truncxor(x_in[j + 4*1], round_tkw[j + 4*1], y[j + 4*1])
                self.constraint_of_equality(x_in[j + 4*2], y[j + 4*2])
                self.constraint_of_equality(x_in[j + 4*3], y[j + 4*3])
            ## Constraints by MixingLayer
            self.constraints_of_mixing_layer(self.permute_nibbles(y), x_out)    
...
########################################################################################################
    ...
    def get_bitwise_state_values_at_round(self, s, r):
        ...
        '''
        v_names = [["%s_%d_%d_%d" % (s, r, nibble_number, bit_number) for bit_number in range(4)] 
                    for nibble_number in range(16)]
        #v_names.reverse()
        temp = [map(self.model.getVarByName, v_names[i]) for i in range(16)]
        v_values = [[str(int(e.Xn)) for e in temp[i]] for i in range(16)]
        v_values = [hex(int("".join(v_values[i]), 2))[2:] for i in range(16)]
    ...
    def retrieve_upper_trail(self, fileobj_diffpath_up):
        ...
        '''  
        cipher_parameters = 'cipher: skinnyrk\n'
        cipher_parameters += 'sweight: 0\n'
        #cipher_parameters += 'rounds: ' + str(self.R1 - self.R) + '\n'
        cipher_parameters += 'rounds: ' + str(self.R1) + '\n'
        cipher_parameters += 'wordsize: 64\n'
        cipher_parameters += 'mode: 2\n'
        cipher_parameters += 'tweaksize: 64\n'
        cipher_parameters += 'keysize: 64\n'
        ...
        for r in range(0, rounds + 1):
            ...
            if (r < rounds):
                ...
                for j in range(16):
                    if (temp[j] == '0'):
                        fileobj_diffpath_up.write("- x%d[%d:%d] : \"0x0\"\n" % (r, 4*j + 3, 4*j))
            else:
                ...
                temp = list(tku1_values)
                for j in range(16):
                    if (temp[j] == '0'):
                        fileobj_diffpath_up.write("- tk00[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))
                temp = list(tku2_values)
                for j in range(16):
                    if (temp[j] == '0'):
                        fileobj_diffpath_up.write("- tk10[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))
    ...
    def retrieve_lower_trail(self, fileobj_diffpath_down):
        ...
        cipher_parameters = 'cipher: skinnyrk\n'
        cipher_parameters += 'sweight: 0\n'
        #cipher_parameters += 'rounds: ' + str(self.R2 - self.R) + '\n'
        ...
        for r in range(0, rounds + 1):
            ...
            if (r < rounds):
                ...
                for j in range(16):
                    if (temp[j] == '0'):
                        #fileobj_diffpath_down.write("- x%d[%d:%d] : \"0x0\"\n" % ((r - self.R), 4*j + 3, 4*j))
                        fileobj_diffpath_down.write("- x%d[%d:%d] : \"0x0\"\n" % (r, 4*j + 3, 4*j))
            else:
        ...
        temp = list(tkl1_values)
        for j in range(16):
            if (temp[j] == '0'):
                fileobj_diffpath_down.write("- tk00[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))
        temp = list(tkl2_values)
        for j in range(16):
            if (temp[j] == '0'):
                fileobj_diffpath_down.write("- tk10[%d:%d] : \"0x0\"\n" % (4*j + 3, 4*j))
    ...
    def solve_model(self):
        ...
        sol_count = 0
        # self.model.Params.PoolSolutions = self.number_of_patterns
        self.model.optimize()
        ...
        if (self.model.Status == GRB.OPTIMAL or self.model.Status == GRB.TIME_LIMIT or self.model.Status == GRB.INTERRUPTED):
            ...
            with open(self.filename_diffpath_up, 'w') as fileobj_diffpath_up, open(self.filename_diffpath_down, 'w') as fileobj_diffpath_down,\
                ...
                while (sol_count < self.number_of_patterns):
        ...
        fileobj_diffpath_up.close()
        fileobj_io_middle.close()
        fileobj_diffpath_down.close()
        return status
