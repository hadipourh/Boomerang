...
void init_prng(int offset) {
    ...
    size_t initial_seed = 10*time(NULL) + 11*(size_t)(offset);
    srand(initial_seed);   // Initialization, should only be called once. int r = rand();
	printf("[+] PRNG initialized to 0x%08llX\n", initial_seed);
}
...
UINT64 send_boomerangs(int R, int ver, int N1, UINT64 N2, UINT64 N3, unsigned char *dp, unsigned char *dc, unsigned char *dk1, unsigned char *dk2)
{
    // Parallel execution
    UINT64 *NUM;
    ...
    free(NUM);
    printf("sum = %llu\n", sum);
    sum_temp = (double)(N1 * N2 * N3) / (double)sum;
...
int main()
{
    ...
    char dp_str[] = "0000000000000200";
    char dc_str[] = "5605060000450605";
 
    char dk1_str[] = "0000000001000000000000000B0000000000000008000000";
    char dk2_str[] = "000000000020000000000000003000000000000000D00000";
    // Expected probability: 2^(-38.84)
    // #######################################################################################################
    // #######################################################################################################
    ...
    int deg1 = 15;
    int deg2 = 20;
    UINT64 N2 = (UINT64)1 << deg1; // Number of bunches per thread: N2 = 2^(deg)
    UINT64 N3 = (UINT64)1 << deg2; // Number of queries per bunch: N3
    ...
    char name[30];
    sprintf(name, "result_%d.txt", PROGRAMNUMBER);
    fic = fopen(name, "w");
...
//g++ rumi-64-192-22r.c -o rumi-64-192-22r -fopenmp -O3 --std=c++11
