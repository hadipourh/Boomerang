...
int boomerang(int r, int ver, unsigned long long N3, unsigned char *dp, unsigned char *dc, unsigned char *dk1, unsigned char *dk2)
{
    ...
    for (UINT64 t = 0; t < N3; t++)
    {
        ...
        for (i = 0; i < (versions[ver][0] >> 3); i++)
        {
            // rand() ix XORed with c4[i] to improve the quality of randomness
            p1[i] = (rand() ^ c4[i]) & 0xff;
            p1_old[i] = p1[i];
        }
        // derive p2
...
void init_prng(int offset) {
    ...
    unsigned int initial_seed = 10*time(NULL) + offset*1000000;
...
int main(int argc, char *argv[])
{
    ...
    char dc_str[] = "8808080000980808";

    char dk1_str[] = "000000000B00000000000000090000000000000005000000";
    char dk2_str[] = "000000000040000000000000006000000000000000A00000";
    // #######################################################################################################
    // #######################################################################################################
