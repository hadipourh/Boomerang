...
class BCT_Analyzer:
    ...
    def compute_probability_4r(self, B11, Cp10, e1, f13):
        num = 0
        print("Time complexity: 2^%f" % math.log(self.size**6 * len(self.SD[B11]) * len(self.SDi[f13])**2 , 2))
        for dp14 in range(self.size):
            for C9 in self.SD[B11]:
                for dp4 in range(self.size):
                    for C13 in range(self.size):
                        for dp11 in range(self.size):
                            for dp12 in range(self.size):
                                for ep131 in self.SDi[f13]:
                                    for ep132 in self.SDi[f13]:
                                        for Dp4 in range(self.size):
                                            num += self.DBCT[B11][dp14] * (self.ddt[B11][C9]**2) * self.bct[C9][dp14] * \
                                                self.bct[Cp10][dp4] * self.DBCT_vd[B11][C13][dp4] * self.ddt[C13][Dp4] * len(self.BDT_star[Dp4][ep131][ep132][dp4]) * \
                                                    self.ddt[dp11][e1] * self.ddt[dp12][e1] * (self.ddt[dp14][ep131] + self.ddt[dp14][ep132]) * self.ddt[ep131][f13] * self.ddt[ep132][f13]
        print(num)
        print("self.size = %d" % self.size)
        if num != 0:
            return (math.log(num, 2) - (15 * 4))
        else:
            return '-inf'

BCT = BCT_Analyzer(S)
print('computation started ...')
print(BCT.compute_probability_4r(B11=0x2, Cp10=0xD, e1=0x5, f13=0x2))
