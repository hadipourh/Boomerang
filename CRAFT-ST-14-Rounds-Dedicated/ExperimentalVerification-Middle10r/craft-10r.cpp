...
#include <iostream>
using namespace std;
#define Nthreads 16 // Number of parallel threads
...
void init_prng(int offset) {
    ...
    unsigned int initial_seed = 100*time(NULL) + 10*offset;
...
UINT64 boomerang(uint8_t* dp, uint8_t* dc, uint8_t r, uint8_t tk[][16], UINT64 N3)
{
    int i;
    UINT64 num = 0;
...
int main() {
    ...
    const int n = 100;//Number of repeatition
    ...
    string dp_str = "A0000000A0000000";
    string dc_str = "0000000000A00000";
    
    // sage: results = r([2^(-25.415037), 2^(-25.678072), 2^(-25.678072), 2^(-24.830075), 2^(-28.000000), 2^(-27.000000), 2^(-25.41503
    // ....: 7), 2^(-28.000000), 2^(-25.192645), 2^(-25.192645)])
    // sage: r.mean(results)
    // [1] 1.825392e-08
    // sage: float(log(1.825392e-08, 2))
    // -25.70721844539246
    // sage: results.summary()
    //      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
    // 3.725e-09 1.024e-08 2.049e-08 1.825e-08 2.515e-08 3.353e-08
