...
string string_state(uint8_t state[16])
{
    string output;
    char temp[8];
    for (int i = 0; i < 16; i++)
    {
        sprintf(temp, "%01x", state[i]);
        output.append(temp);
    }
    return output;
}
...
UINT64 boomerang(uint8_t key0[16], uint8_t key1[16], uint8_t master_tweak[16], uint8_t tk[][16], uint8_t* dp, uint8_t* dc, uint8_t r, UINT64 N3)
{
	for (uint8_t i = 0; i < 16; i++)
		tk[0][i] = (key0[i] & 0xf);
	for (uint8_t i = 0; i < 16; i++)
	...
	for (uint8_t i = 0; i < 16; i++)
		tk[3][i] ^= (master_tweak[Q[i]] & 0xf);
    UINT64 num = 0;
    for(uint8_t i = 0; i < 16; i++) dp[i] = dp[i] & 0xf;
    for(uint8_t i = 0; i < 16; i++) dc[i] = dc[i] & 0xf;
    uint8_t p1[16],p2[16];
    uint8_t p3[16],p4[16];
    uint8_t c1[16],c2[16];
    uint8_t c3[16],c4[16];
	for (UINT64 t = 0; t < N3; t++){
        bool flag;
		// randomly choose p1
		for(uint8_t i = 0; i < 16; i++) p1[i] = rand() & 0xf;
		// derive p2
		for(uint8_t i = 0; i < 16; i++) p2[i] = p1[i]^dp[i];
        enc(r, p1, c1, tk);
		...
		// derive c3
		for(uint8_t i = 0; i < 16; i++) c3[i] = (c1[i] & 0xf)^dc[i];
		// derive c4
		for(uint8_t i = 0; i < 16; i++) c4[i] = (c2[i] & 0xf)^dc[i];
        dec(r, p3, c3, tk);
		...
		if (flag) {
            ...
            printf("p1: %s\n", string_state(p1).c_str());
            printf("p2: %s\n", string_state(p2).c_str());
            printf("p3: %s\n", string_state(p3).c_str());
            printf("p4: %s\n", string_state(p4).c_str());
            printf("c1: %s\n", string_state(c1).c_str());
            printf("c2: %s\n", string_state(c2).c_str());
            printf("c3: %s\n", string_state(c3).c_str());
            printf("c4: %s\n", string_state(c4).c_str());
            printf("k0: %s\n", string_state(key0).c_str());
            printf("k1: %s\n", string_state(key1).c_str());
            printf("T: %s\n", string_state(master_tweak).c_str());
            // right_quartets_file.open("right_quartets.txt");
            // right_quartets_file << "p1: ";
            // right_quartets_file << string_state(p1);
            // right_quartets_file << "\n";
            // right_quartets_file << "p2: ";
            // right_quartets_file << string_state(p2);
            // right_quartets_file << "\n";
            // right_quartets_file << "p3: ";
            // right_quartets_file << string_state(p3);
            // right_quartets_file << "\n";
            // right_quartets_file << "p4: ";
            // right_quartets_file << string_state(p4);
            // right_quartets_file << "\n";
            // right_quartets_file << "c1: ";
            // right_quartets_file << string_state(c1);
            // right_quartets_file << "\n";
            // right_quartets_file << "c2: ";
            // right_quartets_file << string_state(c2);
            // right_quartets_file << "\n";
            // right_quartets_file << "c3: ";
            // right_quartets_file << string_state(c3);
            // right_quartets_file << "\n";
            // right_quartets_file << "c4: ";
            // right_quartets_file << string_state(c4);
            // right_quartets_file << "\n";
            // right_quartets_file << "k0: ";
            // right_quartets_file << string_state(key0);
            // right_quartets_file << "\n";
            // right_quartets_file << "k1: ";
            // right_quartets_file << string_state(key1);
            // right_quartets_file << "\n";
            // right_quartets_file << "Tweak: ";
            // right_quartets_file << string_state(master_tweak);
            // right_quartets_file << "\n";
            // right_quartets_file << "#########################\n";
            // right_quartets_file.close();
        }
	}
	return num;
}
...
double verify(uint8_t dp[16], uint8_t dc[16], uint8_t R, int N1, UINT64 N2, UINT64 N3) {
	...
	for(counter = 0; counter < N1; counter++){
		...
		for(UINT64 j = 0; j < N2; j++){
			...
			num += boomerang(key0, key1, master_tweak, tk, dp, dc, R, N3);
        }
...
int main(int argc, char *argv[]) {
    ...
    const int n = 1;//Number of repetitions
    uint8_t R = 12;//Number of rounds
    int N1 = Nthreads;
    int deg1 = 14;
    int deg2 = 14;
	UINT64 N2 = 1 << deg1; // 2^(deg1): Number of bunches per threads
	UINT64 N3 = 1 << deg2; // 2^(deg2): Number of queries per bunches
    string dp_str = "00AA000A0AA0000A";
    string dc_str = "00000A000000A000";
    //#################################################################################
    uint8_t dp[16];
    uint8_t dc[16];
    bool reversed = false;
